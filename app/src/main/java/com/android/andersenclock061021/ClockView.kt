...
class ClockView : View {
    ...
    private fun initClock() {
        ...
        val min = Math.min(height0, width0)
    ...
    private fun drawHand(canvas: Canvas, loc: Double, isHour: Boolean) {
        ...
        canvas.drawLine(
            (width0 / 2).toFloat(), (height0 / 2).toFloat(),
            (width0 / 2 + Math.cos(angle) * handRadius).toFloat(),
            (height0 / 2 + Math.sin(angle) * handRadius).toFloat(),
            paint!!
        )
    }
    ...
    private fun drawNumeral(canvas: Canvas) {
        ...
        for (number in numbers) {
            ...
            val x = (width0 / 2 + Math.cos(angle) * radius - rect.width() / 2).toInt()
            val y = (height0 / 2 + Math.sin(angle) * radius + rect.height() / 2).toInt()
    ...
    private fun drawCenter(canvas: Canvas) {
        ...
        canvas.drawCircle((width0 / 2).toFloat(), (height0 / 2).toFloat(), 12f, paint!!)
    }
    ...
    private fun drawCircle(canvas: Canvas) {
        ...
        canvas.drawCircle(
            (width0 / 2).toFloat(), (height0 / 2).toFloat(), (radius + padding - 10).toFloat(),
            paint!!
        )
    }
}